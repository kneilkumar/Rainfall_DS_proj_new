...
for dfs in [mangere_df, albany_df, heads_df, motat_df]:
    ...
    dfs["True Rainfall [mm]"] = dfs["Rainfall [mm]"]

# ------------- pred vs true ------------ #

st.write("# Prediction vs Reality")
st.subheader("How well the model did", divider="rainbow")
...
with st.expander("How to view data"):
    st.markdown('1. Click on the button below, it should take you to my Github repo ')
    st.markdown('2. Navigate to the file called auckland_rain_map.html')
    st.markdown('3. Click on the file and it should give you the option to download it.')
    st.markdown('4. Once downloaded, you can open it in your browser.')
    st.markdown('5. All the station data will not display right off the bat, so click on the button in the top right corner.')
    st.markdown('6. Check all the boxes off, and then turn them back on.  You should be able to toggle which weather station(s) is displaying data.')
    st.markdown('7. Enjoy!')
...
with st.expander("Setup Explanation"):
    st.write('''the method with which the folium plugin interacts with streamlit doesn't work and is known to cause bugs,
    this is the best workaround which still uses that plugin.  ''')
