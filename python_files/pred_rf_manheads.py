...
def load_data(file_name_list):
    master_df = pd.read_csv(file_name_list[0],nrows=30346)
...
# LOAD DATA
data = ['manukau_heads_rain.csv',
        'manukau_heads_sunshine.csv',
        'manukau_heads_temperature.csv',
        'manukau_heads_wind.csv',
        'manukau_heads_pressure.csv',]
...
for name in ["test_set_manheads.csv", "train_set_manheads.csv"]:
    fpf = os.path.join("/Users/neilkumar/Desktop/Python/Oceanum", name)
    if os.path.isfile(fpf):
        pass
    else:
        train_master.to_csv("train_set_manheads.csv")
        test_master.to_csv("test_set_manheads.csv")
...
train_master = read_extra_rain(['motat_rain.csv',
                               'mangere_rain.csv',
                               'albany_rain.csv'], train_master)


test_master = read_extra_rain(['motat_rain.csv',
                               'mangere_rain.csv',
                               'albany_rain.csv'], test_master)

train_master = read_extra_wind(['motat_wind.csv',
                                'mangere_wind.csv',
                                'albany_wind.csv'], train_master)

test_master = read_extra_wind(['motat_wind.csv',
                                'mangere_wind.csv',
                               'albany_wind.csv'], test_master)

train_master = read_extra_pressure(['motat_pressure.csv',
                                'mangere_pressure.csv',
                                'albany_pressure.csv'], train_master)

test_master = read_extra_pressure(['motat_pressure.csv',
                                'mangere_pressure.csv',
...
train_master.to_csv("train_set_manheads.csv")
fpf = os.path.join("/Users/neilkumar/Desktop/Python/Oceanum","train_set_manheads.csv" )
if os.path.isfile(fpf):
    pass
else:
    train_master.to_csv("train_set_manheads.csv")
...
fpf = os.path.join("/Users/neilkumar/Desktop/Python/Oceanum","test_set_manheads.csv" )
if os.path.isfile(fpf):
    pass
else:
    test_master.to_csv("test_set_manheads.csv")
...
def int_feats(train_num, test_num):
    ...
    train_num['P_grad_man_motat'] = train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_motat_pressure']
    train_num['P_grad_man_mang'] = train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_mangere_pressure']
    train_num['P_grad_man_alb'] = train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_albany_pressure']
    test_num['P_grad_man_motat'] = test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_motat_pressure']
    test_num['P_grad_man_mang'] = test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_mangere_pressure']
    test_num['P_grad_man_alb'] = test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_albany_pressure']
    train_num['P_grad_man_motat_t'] = (train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_motat_pressure'])/6
    train_num['P_grad_man_mang_t'] = (train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_mangere_pressure lag: 6'])/6
    train_num['P_grad_man_alb_t'] = (train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_albany_pressure lag: 3'])/3
    test_num['P_grad_man_motat_t'] = (test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_motat_pressure lag: 3'])/3
    test_num['P_grad_man_mang_t'] = (test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_mangere_pressure lag: 6'])/6
    test_num['P_grad_man_alb_t'] = (test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_albany_pressure lag: 3'])/3
    test_num['dewtemp'] = test_num['dewpoint (degC)'] * test_num['Mean Temperature [Deg C]']
...
lag_feat = ["Rainfall [mm]", "Mean sea level pressure [Hpa]", "Mean Relative Humidity [percent]",
            ...
            "Rainfall_motat_rain", "Rainfall_mangere_rain",
            "Rainfall_albany_rain","Speed_motat_wind", "Speed_mangere_wind"
            , "Sea_level_pressure_mangere_pressure","Sea_level_pressure_albany_pressure",
            "Sea_level_pressure_motat_pressure"]
roll_feat = lag_feat

pressure_list = ["Sea_level_pressure_mangere_pressure","Sea_level_pressure_albany_pressure","Sea_level_pressure_motat_pressure" ]
lag_list = ["Sea_level_pressure_mangere_pressure lag: 6","Sea_level_pressure_albany_pressure lag: 6",
            "Sea_level_pressure_motat_pressure lag: 6"]
...
train_reg_1 = XGBRegressor(n_estimators=5000, max_depth=4, learning_rate=0.065, verbosity=1,
    objective='reg:absoluteerror', booster='gbtree', tree_method='hist', gamma=0.075, min_child_weight=50, colsample_bytree=0.4,
    reg_lambda=0.05, random_state=42, subsample=0.95,n_jobs=-1)

reg_names = ["control regressor", "test_regressor"]
i = 0
sample_prt = prt_fm.sample(n=3000, random_state=42)
sample_tgt = pure_rain_train[sample_prt.index]
...
reg_imp.index = [i for i in range(201)]
reg_imp.to_csv("regimp_heads.csv")
...
class_imp.index = [i for i in range(201)]
class_imp.to_csv("classimp_heads.csv")
X_train_class = X_train.drop(list(class_imp["feature_name"])[193:], axis=1)
classifier_score = cross_val_score(trained_classifier, X_train_class, y_train_bin, cv=10,scoring='f1',n_jobs=-1)
print("classification:", classifier_score.mean())
...
test_master['regressionss'] = list(oos_reg_pred)
test_master['final_preds'] = list(final_oos_pred)
test_master.to_csv("csv_manheads_for_dash")
