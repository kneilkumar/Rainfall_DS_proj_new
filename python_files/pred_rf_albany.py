...
def load_data(file_name_list):
    master_df = pd.read_csv(file_name_list[0],nrows=136637)
...
# LOAD DATA
data = ['albany_rain.csv',
        'albany_sunshine.csv',
        'albany_temperature.csv',
        'albany_wind.csv',
        'albany_pressure.csv',]
...
for name in ["test_set_albany.csv", "train_set_albany.csv"]:
    fpf = os.path.join("/Users/neilkumar/Desktop/Python/Oceanum", name)
    if os.path.isfile(fpf):
        pass
    else:
        train_master.to_csv("train_set_albany.csv")
        test_master.to_csv("test_set_albany.csv")
...
train_master.to_csv("train_set_motat.csv")
fpf = os.path.join("/Users/neilkumar/Desktop/Python/Oceanum","train_set_albany.csv" )
if os.path.isfile(fpf):
    pass
else:
    train_master.to_csv("train_set_albany.csv")
...
fpf = os.path.join("/Users/neilkumar/Desktop/Python/Oceanum","test_set_albany.csv" )
if os.path.isfile(fpf):
    pass
else:
    test_master.to_csv("test_set_albany.csv")
...
def int_feats(train_num, test_num):
    ...
    train_num['P_grad_alb_man'] = train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_manukau_heads_pressure']
    train_num['P_grad_alb_mang'] = train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_mangere_pressure']
    train_num['P_grad_alb_motat'] = train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_motat_pressure']
    test_num['P_grad_alb_man'] = test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_manukau_heads_pressure']
    test_num['P_grad_alb_mang'] = test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_mangere_pressure']
    test_num['P_grad_alb_motat'] = test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_motat_pressure']
    train_num['P_grad_alb_man_t'] = (train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_manukau_heads_pressure'])/6
    train_num['P_grad_alb_mang_t'] = (train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_mangere_pressure lag: 6'])/6
    train_num['P_grad_alb_motat_t'] = (train_num['Mean sea level pressure [Hpa]'] - train_num['Sea_level_pressure_motat_pressure lag: 3'])/3
    test_num['P_grad_alb_man_t'] = (test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_manukau_heads_pressure lag: 3'])/3
    test_num['P_grad_alb_mang_t'] = (test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_mangere_pressure lag: 6'])/6
    test_num['P_grad_alb_motat_t'] = (test_num['Mean sea level pressure [Hpa]'] - test_num['Sea_level_pressure_motat_pressure lag: 3'])/3
    test_num['dewtemp'] = test_num['dewpoint (degC)'] * test_num['Mean Temperature [Deg C]']
...
lag_feat = ["Rainfall [mm]", "Mean sea level pressure [Hpa]", "Mean Relative Humidity [percent]",
            ...
            "Rainfall_manukau_heads_rain", "Rainfall_mangere_rain",
            "Rainfall_motat_rain","Speed_manukau_heads_wind", "Speed_mangere_wind",
            "Speed_motat_wind", "Sea_level_pressure_mangere_pressure",
            "Sea_level_pressure_motat_pressure", "Sea_level_pressure_manukau_heads_pressure"]
roll_feat = lag_feat

pressure_list = ["Sea_level_pressure_motat_pressure", "Sea_level_pressure_mangere_pressure","Sea_level_pressure_manukau_heads_pressure"]
lag_list = ["Sea_level_pressure_mangere_pressure lag: 6","Sea_level_pressure_motat_pressure lag: 6", "Sea_level_pressure_manukau_heads_pressure lag: 6"]
...
train_reg_1 = XGBRegressor(n_estimators=5000, max_depth=4, learning_rate=0.065, verbosity=1,
    objective='reg:absoluteerror', booster='gbtree', tree_method='hist', gamma=8, min_child_weight=0.005, colsample_bytree=0.85,
    reg_lambda=0.01, random_state=42, subsample=0.95,n_jobs=-1)
...
classifier_score = cross_val_score(XGBClassifier(scale_pos_weight=scale_pos_weight,
                                                 n_estimators=190, min_child_weight=10,
                                                 max_depth=4,subsample=0.95,
                                                 learning_rate=0.04, gamma=15,
                                                 eval_metric='logloss', colsample_bytree=0.95, random_state=42
...
class_imp.to_csv("classimp_alb.csv")
...
oos_class_pred = whole_class.predict(X_test)
...
test_master.to_csv("csv_albany_for_dash")
